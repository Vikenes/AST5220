...
In order to gain insight into the impact of different physical effects on the power spectrum, we can examine the individual components of the source function. We will provide such an analysis of the individual components of $S(k,x)$, and their resulting $\cl$, in \secref{sssec:M4:results:angular_power_spectrum}. Before delving into those details, we begin by presenting a general overview of each term.
