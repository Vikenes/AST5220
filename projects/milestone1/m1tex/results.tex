...
\subsubsection{Analytical and numerical comparisons} \label{sssec:M1:results:analytical_and_numerical_comparisons}
...
\subsubsection{Hubble factor and time variables} \label{sssec:M1:results:Hubble_factor_and_time_variables}
...
\subsubsection{Supernova fitting} \label{sssec:M1:results:supernova_fitting}
The best fit parameters are given in \tabref{tab:M1:results:best_fit_supernova}, where the standard deviation of each parameter is included as a measure of the uncertainty. For $\omn$ and $\okn$, we see that the best fit values are within one standard deviation from the measured Planck values. For $H_0$ on the other hand, this is not the case. The best fit yields $\chi^2_\mathrm{min}/N=0.945$. This is close to unity, meaning that the fit is good. However, since it's smaller than one, this indicates that we are somewhat overfitting the data, but to confirm this, further analysis on the data is needed. 
\tablesMilestoneOne{best_fit_supernova.tex} 

\figref{fig:M1:results:dL_z_compare_fitted} shows three luminosity distances, with observational data shown in red. The two curves shows the luminosity distance computed with Planck cosmology, dashed green line, and the result from using the parameters in \tabref{tab:M1:results:best_fit_supernova}, solid blue line. We plot $d_L(z)/z$ to better compare the simulations with the data. There is a noticeable discrepancy between the data and the Planck cosmology, with most of the simulated values lying outside the measurement uncertainties. The simulation with the best fit parameters yields a luminosity distance function that is much more consistent with data. However, there is still some discrepancy at lower redshift, but the associated data points in this regime have large uncertainties.    
\begin{figure}[ht!]
    \includefig{dL_z_compare_fitted}
    \caption{The luminosity distance function obtained from the best fit values of $\C$ (blue line), compared with the result from the Planck cosmology (dashed green line).}
    \label{fig:M1:results:dL_z_compare_fitted}
\end{figure}

The $1\sigma$ and $2\sigma$ confidence regions in the $\ol-\om$ plane is shown in \figref{fig:M1:results:mcmc_supernova_fit_Nburn1000}. In the figure we have also indicated the parameter configuration for a flat Universe. From this plot, it's evident that Planck cosmology is within $1\sigma$ region for the two density parameters, as the $1\sigma$ region covers a relatively large configuration space.        
\begin{figure}[ht!]
...
The posterior PDF of $H_0$ is shown in \figref{fig:M1:results:H0_pdf_Nburn1000}, where we have included the resulting Gaussian distribution from the mean and variance of the sampled $H_0$ values. This shows distinct discrepancy from the value of $H_0=67\unit{km/s/Mpc}$, given by Planck. The minimum value of $H_0$ included in the accepted parameters is $\min(\hat{H}_0)=67.58\unit{km/s/Mpc}$, meaning that the Planck value is not included at all.       
\begin{figure}[ht!]
...
\begin{table*}[ht!]
    \raggedright
    \input{../milestone1/m1tex/tables/time_values.tex}
